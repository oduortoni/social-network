...
func (s *groupChatMessageService) SendGroupChatMessage(groupID, senderID int, content string) (*models.GroupChatMessage, error) {
	_, err := s.groupService.GetGroupByID(groupID)
	if err != nil {
		return nil, fmt.Errorf("group not found: %w", err)
	}

	isMember, err := s.groupMemberStore.IsGroupMember(groupID, senderID)
	if err != nil {
...
func (s *groupChatMessageService) GetGroupChatMessages(groupID int, userID int, limit, offset int) ([]*models.GroupChatMessage, error) {
	_, err := s.groupService.GetGroupByID(groupID)
	if err != nil {
		return nil, fmt.Errorf("group not found: %w", err)
	}
